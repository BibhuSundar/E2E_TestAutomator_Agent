...
export default function PlanningPage() {
  ...
  const [strategyJiraUploading, setStrategyJiraUploading]     = useState(false)
  const [strategyJiraUploadResult, setStrategyJiraUploadResult] = useState(null)
  const [strategyJiraUploaded, setStrategyJiraUploaded]       = useState(false)
  ...
  const handleGenerateStrategy = async () => {
    const task = buildTask()
    if (!task) return
    setStrategyLoading(true); setError(''); setStrategyResult(null); setStrategyApproved(false); setStrategyApprovedAt(null); setStrategyRejected(false); setStrategyRejectedAt(null); setStrategyJiraUploaded(false); setStrategyJiraUploadResult(null)
    try {
  ...
  const handleStrategyUploadToJira = async () => {
    ...
    if (!key) {
      setStrategyJiraUploadResult({ success: false, error: 'Jira Project Key is not configured. Go to Settings → Jira Configuration to set it up.' })
      return
    }
    setStrategyJiraUploading(true)
    setStrategyJiraUploadResult(null)
    try {
      const data = await jiraAPI.createIssue(
        key,
        `Test Strategy generated by Test Automator - ${new Date().toLocaleString()}`,
        strategyResult
      )
      setStrategyJiraUploadResult({ success: true, key: data.issue_key, url: data.issue_url })
      setStrategyJiraUploaded(true)
    } catch (err) {
      setStrategyJiraUploadResult({ success: false, error: err.response?.data?.detail || 'Failed to create Jira issue.' })
    } finally {
      setStrategyJiraUploading(false)
    }
  }
  ...
  const handleUploadToJira = async () => {
    ...
    try {
      const data = await jiraAPI.createIssue(
        key,
        `Test Planning generated by Test Automator - ${new Date().toLocaleString()}`,
        result
      )
  ...
  const switchTab = (id) => {
    ...
    setStrategyJiraUploaded(false); setStrategyJiraUploadResult(null)
  }
  ...
  return (
    <DashboardLayout title="Planning">
      <div style={s.page}>
            ...
            <div style={{ padding: '14px 18px', fontSize: '0.85rem', color: '#374151' }}>
              {jiraUploadResult.success ? (
                <span>
                  ...
                  <a href={jiraUploadResult.url} target="_blank" rel="noopener noreferrer" style={{ color: '#0891b2', fontWeight: 600 }}>
                    Open in Jira ↗
                  </a>
                </span>
              ) : (
        ...
        {strategyJiraUploadResult && (
          <div style={{
            ...s.resultCard,
            borderColor: strategyJiraUploadResult.success ? '#bbf7d0' : '#fecaca',
          }}>
            <div style={{
              ...s.resultHead,
              background: strategyJiraUploadResult.success ? 'rgba(34,197,94,0.05)' : 'rgba(239,68,68,0.05)',
              borderColor: strategyJiraUploadResult.success ? '#bbf7d0' : '#fecaca',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '1rem' }}>{strategyJiraUploadResult.success ? '✅' : '❌'}</span>
                <span style={{ ...s.resultTitle, color: strategyJiraUploadResult.success ? '#166534' : '#dc2626' }}>
                  {strategyJiraUploadResult.success ? 'Uploaded to Jira' : 'Upload Failed'}
                </span>
              </div>
            </div>
            <div style={{ padding: '14px 18px', fontSize: '0.85rem', color: '#374151' }}>
              {strategyJiraUploadResult.success ? (
                <span>
                  Issue <strong>{strategyJiraUploadResult.key}</strong> created successfully.{' '}
                  <a href={strategyJiraUploadResult.url} target="_blank" rel="noopener noreferrer" style={{ color: '#0891b2', fontWeight: 600 }}>
                    Open in Jira ↗
                  </a>
                </span>
              ) : (
                <span style={{ color: '#dc2626' }}>{strategyJiraUploadResult.error}</span>
              )}
            </div>
            <div style={s.resultFoot}>
              <button
                style={{ ...s.clearBtn, fontSize: '0.75rem', padding: '4px 12px', width: 'auto', height: 'auto' }}
                onClick={() => setStrategyJiraUploadResult(null)}
              >Dismiss</button>
            </div>
          </div>
        )}
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                ...
                <button
                  style={{
                    ...s.jiraUploadBtn,
                    opacity: !strategyApproved || strategyJiraUploading || strategyJiraUploaded ? 0.55 : 1,
                    cursor: !strategyApproved || strategyJiraUploading || strategyJiraUploaded ? 'not-allowed' : 'pointer',
                  }}
                  onClick={handleStrategyUploadToJira}
                  disabled={!strategyApproved || strategyJiraUploading || strategyJiraUploaded}
                >
                  {strategyJiraUploading ? <><span style={s.spin}>⟳</span> Uploading...</> : strategyJiraUploaded ? '☁️ Uploaded' : '☁️ Upload Jira'}
                </button>
